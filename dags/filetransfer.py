...
dag = DAG(
    dag_id = 'SFTP_File_Xfer',
    description = 'SFTP_File_Xfer',
    default_args = args,
    max_active_runs = 1,
    concurrency = 10,
    catchup = False,
...
for i in dag_config['SFTP_Polling_Sites']:
    ...
    sftpConnName = i['SFTP_Connection_Name']

    for j in i['Feed_Groups']:
        tempDict = {'SFTP_Connection_Name': i['SFTP_Connection_Name'], 'Feed_Group_Name': j['Feed_Group_Name']}
        # Add the Feed Groups Dictionary to tempDict to pass to python callable.
        tempDict.update(j)

        task = PythonOperator(
            task_id = 'Get_{}_files_from_{}_site'.format(j['Feed_Group_Name'], i['SFTP_Name']),
            python_callable = ,
            op_kwargs = tempDict,
            dag = dag
        )
        ...
        task2 = PythonOperator(
            task_id = 'Pull_{}_Files'.format(j['Feed_Group_Name']),
            python_callable = ,
            op_kwargs = tempDict,
            dag = dag
        )
