...
numFiles = int(Variable.get('numFiles'))
...
for i in range(0, numFiles):
    fileName = f'testfile_{random.randint(0,10000)}.txt'

    task = SSHOperator(
        task_id=f'CreateFile{i}',
        ssh_hook=sshHook,
        command=f'touch {fileName} && echo {fileName}',
        do_xcom_push=True,
        dag=dag
    )
...
list_arch_directory = SSHOperator(
...
for i in range(0, numFiles):
    task = SSHOperator(
        task_id=f'MoveFile{i}',
        ssh_hook=sshHook,
        command=f'mv ./{{{{ ti.xcom_pull(task_ids="CreateFile{i}", key="return_value").decode("UTF-8").strip() }}}} ./archive/',
        do_xcom_push=True,
        dag=dag
    )

    list_directory >> task
    task >> list_arch_directory
